...
# GLOBAL REMARK NAME
# =========================
REMARK_NAME = "V2Ray Server Store"
...
def parse_vmess(link):
    try:
        ...
        if d.get("tls")=="tls":
            ...
            obj["streamSettings"]["tlsSettings"]={
                "serverName":d.get("sni") or d.get("host") or d["add"],
                "allowInsecure":True
            }
...
def parse_vless(link):
    try:
        ...
        addr=ps[0]
        port=int(ps[1]) if len(ps)>1 else 443
        ...
        if fl:
            ob["settings"]["vnext"][0]["users"][0]["flow"]=fl
        ...
        if sec=="tls":
            ...
            if fp:
                ob["streamSettings"]["tlsSettings"]["fingerprint"]=fp
        ...
        if net=="ws":
            ...
            ob["streamSettings"]["grpcSettings"]={
                "serviceName":pm.get('serviceName','')
            }
...
def main():
    ...
    def worker(idx_link):
        ...
        with lock:
            ...
            if lat > 0:
                fixed_link = add_remark(link)

                working.append(fixed_link)
